...
class FeedMetadata(BaseModel):
    """lightweight feed metadata for display purposes."""
...
async def fetch_feed_metadata(
    feed_url: str,
    client: httpx.AsyncClient | None = None,
    timeout: float = 15.0,
) -> FeedMetadata | None:
    """fetch lightweight metadata from an rss feed.

    this is optimized for quick metadata retrieval (title, author, episode count)
    without the overhead of parsing all episode details.
    ...
    returns:
        FeedMetadata if successful, None if fetch fails.
    """
    should_close = client is None
    if client is None:
        client = httpx.AsyncClient(
    ...
    try:
        ...
        # count episodes with audio enclosures.
        episode_count = sum(
            1 for entry in feed.entries
            if any(
                enc.get("type", "").startswith("audio/") or
                enc.get("href", "").endswith((".mp3", ".m4a", ".ogg", ".wav"))
