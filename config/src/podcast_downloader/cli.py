...
def select_feed_interactive(
    ...
    # find best match (highest confidence) and most episodes.
    best_match_idx = max(range(len(display_feeds)), key=lambda i: display_feeds[i].confidence)
    most_episodes_idx = max(
        range(len(display_feeds)),
        key=lambda i: display_feeds[i].episode_count if display_feeds[i].episode_count else 0
    )
    ...
    for idx, feed in enumerate(display_feeds, start=1):
        ...
        # add indicators for best match and most episodes.
        title_display = feed.title[:35] if feed.title else "Unknown"
        indicators = []
        
        if idx - 1 == best_match_idx:
            indicators.append("★")  # best fuzzy match.
        if idx - 1 == most_episodes_idx and feed.episode_count:
            indicators.append("📁")  # most episodes.
        
        if indicators:
            # truncate title to make room for indicators.
            max_title_len = 35 - len(" ".join(indicators)) - 1
            title_display = title_display[:max_title_len] + " " + " ".join(indicators)
    ...
    console.print("[dim]★ Best match  📁 Most episodes[/dim]")
    console.print()
...
def parse_date(date_str: str) -> datetime:
    ...
    # parse naive datetime and make it timezone-aware (UTC) to match feed dates.
...
def download(
    ...
    if feed_url:
        ...
        for podcast_name in podcast_names:
            ...
            feeds = asyncio.run(_discover_feeds_async(podcast_name, limit=max(matches, 10)))
